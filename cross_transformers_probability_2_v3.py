...
class CrossTransformer_score2(nn.Module):
    ...
    def calcScore(self, supports_repr, query_repr):
        ...
        # supports_repr_avg(25,512,1,1)

        supports_repr_avg_mean = torch.mean(supports_repr_avg, dim=0) #对每张图片取mean
        #supports_repr_avg_mean(512,1,1)

        supports_repr_avg_repeat = supports_repr_avg_mean.repeat(1, 7, 7) # 改变维度，下面才可以进行concat操作
        # supports_repr_avg_repeat(512,7,7)

        supports_repr_avg_repeat_unsqueeze = supports_repr_avg_repeat.unsqueeze(dim=0) # 改变维度，下面才可以进行concat操作
        # supports_repr_avg_repeat_unsqueeze(1,512,7,7)

        supports_repr_avg_repeat_unsqueeze_repeat = supports_repr_avg_repeat_unsqueeze.repeat(len(query_repr),1,1,1) # 改变维度，下面才可以进行concat操作
        ...
        query_feat_weighted = sigmoidScoreFeat.mul(query_repr)
        # weightSupportFeat(25,512,7,7)

        return query_feat_weighted
    ...
    def forward(self, model, img_query, img_supports):
        ...
        *_, h, w = query_repr.shape
        ...
        query_feat_weighted = self.calcScore(supports_repr, query_repr).cuda()
        # query_feat_weighted(25,512,7,7)

        query_repr = query_feat_weighted

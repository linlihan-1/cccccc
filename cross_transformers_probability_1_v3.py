...
class CrossTransformer_score1(nn.Module):
    ...
    def calcScore(self, support_feat, query_feat):
        ...
        for i in range(len(support_feat)):
            prob = normal.log_prob(query_feat[i]) # support_feat改query_feat
        ...
        #
        # attention_mask_sig = torch.sigmoid(attention_mask_norm) #等问下老师，对每张图片做sigmoid什么意思
        ...
        attention_map = query_feat.view(25,512,7,7) * attention_mask_soft
        # attention_map(1225,512)

        query_feat_weighted = attention_map.view(25,7,7,-1).type(torch.FloatTensor)
    ...
    def forward(self, model, img_query, img_supports):
        ...
        *_, h, w = query_repr.shape
        ...
        query_weighted = self.calcScore(supports_repr, query_repr).cuda()
        # query_weighted(25,7,7,512)

        query_repr = rearrange(query_weighted, 'nk h w c -> nk c h w')

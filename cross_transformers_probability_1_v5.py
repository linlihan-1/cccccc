...
class CrossTransformer_score1(nn.Module):
    ...
    def calcScore(self, support_feat, query_feat):
        support_feat_re = rearrange(support_feat, "b c w h -> (b w h) c").double()
        # support_feat(25,512,7,7) ->(1225, 512)

        query_feat_re = rearrange(query_feat, "b c w h -> (b w h) c").double()
        # support_feat(25,512,7,7) ->(1225, 512)

        support_feat_mean = torch.mean(support_feat_re, dim=0).double()
        # support_feat_mean(512,)

        support_feat_cov = torch.cov(support_feat_re.T).double()
        # support_feat_cov(512,512)

        ###########make sure PSD#############################
        # support_feat_cov = torch.mm(support_feat_cov, support_feat_cov.t()).cuda()
        support_feat_cov = support_feat_cov.add(0.01 * torch.eye(len(support_feat_cov)).cuda()).cuda()
        #######################################
        # print(support_feat_cov)
        normal = dist.MultivariateNormal(support_feat_mean, support_feat_cov)

        attention_mask = torch.zeros(len(query_feat_re)).double().cuda()
        # attention_mask(nk,)

        ##################计算nk个local feature的概率##############
        for i in range(len(query_feat_re)):
            prob = normal.log_prob(query_feat_re[i]).exp() # support_feat改query_feat
            if prob == 0:
        ...
        attention_mask_soft = softmax_dim0(attention_mask_norm.view(25,1,len(query_feat[0][0]),len(query_feat[0][0])) * 10) #对第一维，也就是每张图片做softmax

...
from mutil_cross_transformers import MultiCrossTransformer
from utils import *
# from dataset.datasets_csv import Imagefolder_csv
from dataset import Imagefolder_csv
# from model import CrossAttention
from cross_transformers import CrossTransformer
from cross_transformers_probability_1_v5 import CrossTransformer_score1
from cross_transformers_probability_2_v4 import CrossTransformer_score2
parser = argparse.ArgumentParser()
parser.add_argument('--base_dir', default='miniImageNet', help='base directory containing csv and images folder')
...
'''
parser.add_argument('--dataset', type=int, default='xxxx')
'''
...
parser.add_argument('--query_num', type=int, default=5, help='the number of queries')
...
parser.add_argument('--basemodel', type=str, default='CrossTransformer', help="model's name")
...
def train(train_loader, model, criterion, optimizer, epoch_index, F_txt):
	...
	for episode_index, (query_images, query_targets, support_images, support_targets) in enumerate(train_loader):
		...
		input_var2 = input_var2.unsqueeze(0)
		# Deal with the targets
		target = torch.cat(query_targets, 0)
		target = target.type(torch.LongTensor)  # <---- Here (casting)
		target = target.cuda()
		...
		with autocast():
			...
			# output = cross_transformer(model, input_var1, input_var2)
			output = cross_transformer(model, input_var1, input_var2)
...
if __name__ == '__main__':
	...
	# ===================resnet12======================
	from backbone import ResNet_12
	model = ResNet_12.resnet12()
	# =================================================

	# ===================conv-4======================
	# from backbone import Conv_4
	# model = Conv_4.BackBone()
	...
	cross_transformer = CrossTransformer_score1(
	# cross_transformer = CrossTransformer_score2(
		dim=512,
		dim_key=128,
		dim_value=128
	)

	# cross_transformer = MultiCrossTransformer(
	# 	dim=512,
	# 	dim_key=128,
	# 	dim_value=128
	# )
	# model.to(device)
	# cross_transformer.to(device)
	model.cuda()
	cross_transformer.cuda()
	# mutil_cross_transformer.cuda()
	################################
